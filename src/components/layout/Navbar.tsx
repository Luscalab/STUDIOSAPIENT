    ...
    <>
      <header className="absolute top-8 left-0 right-0 z-[150]" role="banner">
        <div className="container mx-auto px-6 flex items-center justify-between gap-12">
          ...
          <Link href="/" className="relative block w-[200px] h-[55px] lg:w-[480px] lg:h-[130px] transition-transform duration-700 hover:scale-105 active:scale-95 shrink-0">
            <Image 
              src={logoUrl}
          ...
          <nav className="hidden lg:flex items-center gap-10 px-10 py-5 rounded-full glass-morphism border-white/5">
            {navLinks.map((link) => (
              <Link 
                ...
                className="text-[11px] uppercase tracking-[0.4em] font-black text-white/50 hover:text-white transition-all relative group"
              >
                {link.name}
            ...
            <button 
              ...
              className="text-[11px] uppercase tracking-[0.4em] font-black text-primary hover:text-white transition-all flex items-center gap-2"
            >
