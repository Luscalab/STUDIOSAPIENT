...
export function FAQ() {
  return (
    <section id="faq" className="py-32 md:py-48 bg-white relative overflow-hidden">
        ...
        <Accordion type="single" collapsible className="w-full space-y-6">
          {faqs.map((faq, idx) => (
            <AccordionItem 
              key={idx} 
              value={`item-${idx}`} 
              className="border-none bg-secondary/50 rounded-[2.5rem] px-10 py-4 shadow-sm border border-muted hover:border-primary/20 transition-all duration-500 overflow-hidden"
            >
              <AccordionTrigger className="text-xl md:text-2xl font-black tracking-tighter hover:no-underline text-left py-6 group text-foreground uppercase leading-tight border-none">
              ...
              <AccordionContent className="text-lg text-foreground/50 leading-relaxed pb-10 font-medium tracking-tight px-6 border-l-2 border-primary/20 ml-6">
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
