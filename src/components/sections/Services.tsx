...
export function Services() {
  return (
    <section id="servicos" className="py-16 md:py-24 lg:py-32 bg-white text-black relative overflow-hidden rounded-[3rem] md:rounded-[6rem] mx-4 my-6 shadow-xl">
      <div className="container mx-auto px-6 relative z-10">
        <div className="mb-12 lg:mb-20 text-center md:text-left max-w-3xl">
          ...
          <CarouselContent className="-ml-4">
            {services.map((service, idx) => (
              <CarouselItem key={idx} className="pl-4 md:basis-1/2 lg:basis-1/3">
                <Card 
                  className="group relative border-none bg-secondary/30 rounded-[2.5rem] p-8 transition-all duration-700 hover:bg-white hover:shadow-2xl flex flex-col justify-between min-h-[460px] lg:min-h-[520px] h-full overflow-hidden border border-muted/5"
                >
                  <div className="relative z-10">
                    <div className="mb-8">
                      <div className="h-14 w-14 rounded-xl bg-primary/5 flex items-center justify-center text-primary transition-all duration-700 group-hover:bg-primary group-hover:text-white border border-primary/10">
                        {service.icon}
                      </div>
                    </div>
                    ...
                    <CardDescription className="text-black/50 text-sm md:text-base leading-relaxed font-medium mb-8 tracking-tight">
                      {service.description}
                    </CardDescription>

                    <div className="space-y-3 mb-10">
                      {service.features.map((feature, fIdx) => (
                        <div key={fIdx} className="flex items-center gap-3 text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] text-black/40 group-hover:text-black/70 transition-colors">
