    ...
    <section id="servicos" className="py-32 md:py-64 bg-white relative overflow-hidden rounded-[4rem] md:rounded-[8rem] mx-4 my-8 shadow-2xl">
      <div className="container mx-auto px-6 relative z-10">
        <div className="mb-40 text-center max-w-6xl mx-auto space-y-16">
          ...
          <p className="text-black/40 text-2xl md:text-5xl font-medium leading-tight tracking-tight max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 max-w-[1400px] mx-auto">
          {services.map((service, idx) => (
            <Card 
              key={idx} 
              className="group relative border-none bg-secondary/50 rounded-[3.5rem] p-12 md:p-20 transition-all duration-1000 hover:-translate-y-8 hover:shadow-[0_60px_120px_-30px_rgba(0,0,0,0.1)] flex flex-col justify-between min-h-[750px] overflow-hidden"
            >
              <div className="absolute top-0 right-0 p-16 opacity-[0.03] text-primary group-hover:scale-125 transition-transform duration-1000">
                {service.icon}
              </div>
              
              <div className="relative z-10">
                <div className="mb-20">
                  <div className="h-28 w-28 rounded-[2.5rem] bg-primary flex items-center justify-center shadow-2xl shadow-primary/30 transition-all duration-1000 group-hover:scale-110 group-hover:rotate-6">
                    {service.icon}
                  </div>
                </div>

                <CardTitle className="font-headline text-4xl md:text-6xl mb-10 tracking-tighter font-black text-black group-hover:text-primary transition-colors duration-700 uppercase leading-none">
                  {service.title}
                </CardTitle>
                <CardDescription className="text-black/60 text-xl md:text-3xl leading-relaxed font-medium mb-16 tracking-tight">
                  {service.description}
                </CardDescription>

                <div className="space-y-8 mb-20">
                  {service.features.map((feature, fIdx) => (
                    <div key={fIdx} className="flex items-center gap-8 text-[12px] font-black uppercase tracking-[0.5em] text-black/30 group-hover:text-black/60 transition-colors">
              ...
              <Link 
                ...
                className="flex items-center justify-between p-12 rounded-[2.5rem] bg-white text-black hover:bg-black hover:text-white transition-all duration-700 text-[11px] font-black uppercase tracking-[0.5em] group/btn border border-muted shadow-sm relative z-10"
              >
