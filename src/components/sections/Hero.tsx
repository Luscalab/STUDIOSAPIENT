  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-32 pb-20 soft-gradient-bg">
      <div className="absolute inset-0 z-0 pointer-events-none">
        <div className="absolute top-[-10%] right-[-5%] w-[80%] h-[80%] bg-primary/5 blur-[100px] rounded-full animate-float opacity-40" />
        <div className="absolute bottom-[-10%] left-[-5%] w-[60%] h-[60%] bg-accent/5 blur-[100px] rounded-full animate-float [animation-delay:5s] opacity-40" />
      </div>
      
      <div className="container relative z-10 mx-auto px-6 text-center">
        <div className="inline-flex items-center gap-3 rounded-full border border-primary/10 bg-white/40 backdrop-blur-xl px-6 py-2.5 text-[10px] font-bold uppercase tracking-[0.3em] text-primary mb-10 animate-fade-in-up">
          <Target className="h-3.5 w-3.5" />
          <span>Estratégia & Resultados Profissionais</span>
        </div>
        
        <h1 className="font-headline text-5xl sm:text-7xl md:text-8xl lg:text-[9rem] font-bold tracking-tighter mb-8 animate-fade-in-up [animation-delay:200ms] text-foreground leading-[0.9] text-balance max-w-6xl mx-auto">
          Sua marca,<br />
        ...
        <div className="flex flex-col sm:row items-center justify-center gap-6 animate-fade-in-up [animation-delay:600ms]">
          ...
          <Link href={behanceUrl} target="_blank" className="w-full sm:w-auto">
            <Button variant="outline" className="w-full sm:w-auto h-16 md:h-20 px-12 md:px-16 text-sm font-bold border-primary/20 hover:bg-primary/5 transition-all rounded-full group uppercase tracking-widest bg-white/20 backdrop-blur-sm">
