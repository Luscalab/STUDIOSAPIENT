        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((step, idx) => (
            <div key={idx} className="group relative">
              <div className="relative z-10 p-12 rounded-[4rem] bg-white/5 backdrop-blur-3xl border border-white/10 transition-all duration-1000 hover:bg-white/15 hover:border-white/25 hover:-translate-y-4 h-full flex flex-col shadow-3xl overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-16 relative z-10">
                  <div className="h-20 w-20 rounded-[2rem] bg-white/10 flex items-center justify-center text-white group-hover:bg-primary group-hover:scale-110 transition-all duration-1000 border border-white/10 shadow-lg">
                    {step.icon}
                  </div>
                  <span className="text-6xl font-black text-white/10 select-none tracking-tighter group-hover:text-white/20 transition-all duration-1000">
                    0{idx + 1}
                  </span>
                </div>

                <div className="space-y-8 relative z-10 flex-1">
                  <h3 className="font-headline text-3xl font-black tracking-tighter text-white group-hover:text-primary transition-colors duration-700">
                    {step.title}
                  </h3>
                  <div className="h-2 w-16 bg-white/20 group-hover:w-full group-hover:bg-primary/50 transition-all duration-[1500ms] rounded-full" />
                  <p className="text-white/40 leading-relaxed font-medium text-lg md:text-xl tracking-tight group-hover:text-white/60 transition-colors duration-700">
                    {step.description}
                  </p>
                </div>

                {idx < steps.length - 1 && (
                  <div className="hidden lg:flex absolute top-1/2 -right-4 translate-y-[-50%] z-20 text-white/20 group-hover:text-primary/40 group-hover:translate-x-6 transition-all duration-1000">
