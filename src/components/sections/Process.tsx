    ...
    <section id="metodologia" className="py-24 md:py-48 bg-white text-black relative overflow-hidden rounded-[4rem] md:rounded-[8rem] mx-4 my-8 shadow-2xl">
      <div className="container mx-auto px-6 relative z-20">
        <div className="text-center mb-24 max-w-4xl mx-auto space-y-6">
          ...
          <p className="text-black/40 text-xl font-medium leading-relaxed tracking-tight max-w-2xl mx-auto italic">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {steps.map((step, idx) => (
            <div key={idx} className="group relative p-12 rounded-[3.5rem] bg-secondary/30 border border-muted transition-all duration-500 hover:-translate-y-4 shadow-xl">
              ...
              <h3 className="font-display text-2xl font-black text-black mb-4 tracking-tighter uppercase leading-none">
                {step.title}
              </h3>
              <p className="text-black/60 text-base leading-relaxed font-medium mb-12 tracking-tight">
                {step.desc}
              </p>
              <span className="text-8xl font-black text-black/5 absolute -bottom-4 -right-2 select-none group-hover:text-primary/5 transition-colors">
                0{idx + 1}
              </span>
            </div>
          ))}
