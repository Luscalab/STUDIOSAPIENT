...
export function AIChat() {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    // Atualiza o histórico local antes de enviar
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', content: "Interessante. Gostaria de aprofundar essa estratégia com nosso consultor sênior via WhatsApp?" }]);
  ...
  return (
    <div className="fixed inset-0 md:inset-auto md:bottom-24 md:right-6 z-[300] w-full md:w-[420px] md:h-[600px] bg-white rounded-none md:rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden border border-muted/20 animate-in slide-in-from-bottom-8 duration-500">
      ...
      <div className="p-6 bg-[#08070b] text-white flex items-center justify-between border-b border-white/5">
        ...
        <button onClick={() => setIsOpen(false)} className="h-8 w-8 rounded-full bg-white/5 flex items-center justify-center hover:bg-white/10">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/30">
        {messages.length === 0 && (
          <div className="p-6 rounded-2xl bg-white border border-slate-200 text-slate-900 font-medium text-sm leading-relaxed">
            Bem-vindo. Me conte um pouco sobre sua marca ou o desafio que você enfrenta hoje.
          </div>
        )}
        ...
        {isLoading && (
          <div className="flex items-center gap-2 text-slate-400">
            ...
            <span className="text-[9px] font-black uppercase tracking-widest">Analisando Estratégia...</span>
          </div>
        )}
        ...
        {showRedirect && (
          <button 
            ...
            className="w-full py-4 bg-green-500 hover:bg-green-600 text-white rounded-xl font-black uppercase tracking-widest text-[9px] flex items-center justify-center gap-2 shadow-lg transition-all"
          >
      ...
      <div className="p-4 bg-white border-t border-slate-100">
        <form onSubmit={handleSendMessage} className="relative">
          ...
          <button 
            ...
            className="absolute right-2 top-1/2 -translate-y-1/2 h-10 w-10 rounded-lg bg-primary text-white flex items-center justify-center disabled:opacity-50"
          >
