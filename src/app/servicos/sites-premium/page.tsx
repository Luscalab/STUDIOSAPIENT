      ...
      <section className="relative pt-32 pb-24 md:pt-48 md:pb-32 overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <Badge className="mb-8 bg-primary/10 text-primary border-primary/20 px-8 py-2 text-[10px] font-black uppercase tracking-[0.4em] rounded-full backdrop-blur-md animate-pulse">
            {t('service_pages.sites.badge')}
          </Badge>
          <h1 className="font-headline text-5xl md:text-8xl font-black text-white tracking-tighter leading-[0.9] mb-12 py-4">
            {t('service_pages.sites.title')} <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-white to-primary italic font-medium block">{t('service_pages.sites.title_italic')}</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-8">
            ...
            <button onClick={() => document.getElementById('manifesto')?.scrollIntoView({behavior:'smooth'})} className="text-white/30 hover:text-white transition-all text-[9px] font-black uppercase tracking-[0.6em] flex items-center gap-4 group">
              {t('service_pages.sites.method')} <ChevronDown className="h-4 w-4 animate-bounce group-hover:text-primary" />
            </button>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6">
          <div className="max-w-5xl mx-auto space-y-16">
            <h2 className="font-headline text-4xl md:text-7xl font-black tracking-tighter leading-[0.9]">
              {t('service_pages.sites.manifesto_title')} <span className="text-primary italic">{t('service_pages.sites.manifesto_italic')}</span>
            </h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
              <blockquote className="text-xl md:text-3xl text-slate-900 font-medium leading-tight tracking-tight italic border-l-[8px] border-primary pl-8">
                "{t('service_pages.sites.manifesto_quote')}"
              </blockquote>
              <div className="space-y-8">
                <p className="text-lg text-slate-600 font-medium leading-relaxed">
                  {t('service_pages.sites.manifesto_p')}
                </p>
      ...
      <section className="py-24 md:py-48 relative">
        <div className="container mx-auto px-6">
          <div className="mb-24 text-center md:text-left max-w-3xl">
            <h3 className="font-headline text-4xl md:text-7xl font-black tracking-tighter leading-none text-white uppercase">{t('service_pages.sites.pillar_title')} <br/>{t('service_pages.sites.pillar_italic')}</h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 max-w-6xl mx-auto">
            <div className="md:col-span-8 p-12 rounded-[3.5rem] bg-white/5 border border-white/10 group hover:bg-white/10 transition-all duration-700 relative overflow-hidden">
               <div className="relative z-10 space-y-8">
                 ...
                 <div className="space-y-4">
                    <h4 className="font-headline text-3xl md:text-5xl font-black tracking-tighter uppercase leading-none">{t('service_pages.sites.p1_title')}</h4>
                    <p className="text-lg text-white/50 font-medium max-w-lg">{t('service_pages.sites.p1_desc')}</p>
                 </div>
               </div>
            </div>
            ...
            <div className="md:col-span-4 p-10 rounded-[3.5rem] bg-primary text-white space-y-8 transition-all duration-700">
               ...
               <div className="space-y-4">
                  <h4 className="font-headline text-2xl md:text-3xl font-black tracking-tighter uppercase leading-none">{t('service_pages.sites.p2_title')}</h4>
                  <p className="text-base text-white/80">{t('service_pages.sites.p2_desc')}</p>
               </div>
            </div>
            ...
            <div className="md:col-span-6 p-10 rounded-[3rem] bg-[#121216] border border-white/5 space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <h4 className="font-bold text-xl tracking-tighter uppercase">{t('service_pages.sites.p3_title')}</h4>
                </div>
                <p className="text-white/40 leading-relaxed text-sm">{t('service_pages.sites.p3_desc')}</p>
            </div>

            <div className="md:col-span-6 p-10 rounded-[3rem] bg-[#121216] border border-white/5 space-y-6">
                <div className="flex items-center gap-4">
                  <Layout className="h-6 w-6 text-primary" />
                  <h4 className="font-bold text-xl tracking-tighter uppercase">{t('service_pages.sites.p4_title')}</h4>
                </div>
                <p className="text-white/40 leading-relaxed text-sm">{t('service_pages.sites.p4_desc')}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6">
          <div className="text-center max-w-4xl mx-auto space-y-12">
            <h3 className="font-headline text-4xl md:text-8xl font-black tracking-tighter leading-[0.9] text-slate-950">
              {t('service_pages.sites.cta_final_title')} <span className="text-primary italic">{t('service_pages.sites.cta_final_italic')}</span>
            </h3>
            <p className="text-xl md:text-2xl text-slate-600 font-medium tracking-tight">
              {t('service_pages.sites.cta_final_p')}
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-8 pt-8">
              <Button onClick={handleOpenChat} className="h-24 px-16 bg-primary text-white hover:bg-primary/90 rounded-full font-black uppercase tracking-[0.4em] text-[12px] transition-all shadow-2xl shadow-primary/30">
                {t('service_pages.sites.cta_final_btn')} <ArrowRight className="ml-4" />
              </Button>
            </div>
          </div>
        </div>
